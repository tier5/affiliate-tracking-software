...
class AdmindashboardController extends ControllerBusinessBase
{
    ...
    public function findAgencies($agency_type_id)
    {
        ...
        if (!is_array($identity)) {
            $this->response->redirect(
                '/session/login?return=/admindashboard/list/' . $agency_type_id
            );
        ...
        if ($agency_type_id == 1) {
            ...
            // Display businesses
            if ($userObj->is_admin) {
                $agencies = \Vokuro\Models\Agency::find(
                    "parent_id = " . \Vokuro\Models\Agency::BUSINESS_UNDER_RV . " OR parent_id > 0"
                );

                foreach ($agencies as $agent) {
                    $usersinfo = \Vokuro\Models\users::find(
                        "agency_id = " .$agent->agency_id
                    );

                    if ($agent->subscription_id > 0) {
                        $subcription_details=\Vokuro\Models\SubscriptionPricingPlan::findFirst(
                            "id = " .$agent->subscription_id
                        ); 
                    ...
                    foreach ($usersinfo as $use) {
                        $subscription = \Vokuro\Models\BusinessSubscriptionPlan::findFirst(
                            "user_id = " . $use->id
                        );
    ...
    public function listAction($agency_type_id = 1)
    {
        ...
        $this->tag->setTitle(
            'Get Mobile Reviews | See All ' . ($agency_type_id == 1 ? 'Agencies' : 'Businesses')
        );

        $this->findAgencies($agency_type_id);
        $this->view->agency_type_id = $agency_type_id;

        $dbAllParentAgencies = \Vokuro\Models\Agency::find(
            'parent_id = ' . \Vokuro\Models\Agency::AGENCY
        );
        ...
        if (empty($objEmploeeunderbusiness)) {
            ...
            if ($dbLocations) {
                foreach ($dbLocations as $objLocation) {
                    $dbLocationReviewSites = \Vokuro\Models\LocationReviewSite::find(
                        "location_id = {$objLocation->location_id}"
                    );

                    // Review invites
                    $dbReviewInvite = \Vokuro\Models\ReviewInvite::find(
                        "location_id = {$objLocation->location_id}"
                    );
                    
                    foreach ($dbReviewInvite as $objReviewInvite) {
                        $dbReviewInviteReviewSite = \Vokuro\Models\ReviewInviteReviewSite::find(
                            "review_invite_id = {$objReviewInvite->review_invite_id}"
                        );
            ...
            $objSuperUser = \Vokuro\Models\Users::findFirst(
                "agency_id = {$BusinessID} AND role='Super Admin'"
            );

            if ($objSuperUser) {
                $objSubscription = \Vokuro\Models\BusinessSubscriptionPlan::findFirst(
            ...
            $dbSharingCodes = \Vokuro\Models\SharingCode::find(
            ...
            $this->session->set(
                "err_msg",
                'This business cannot be deleted as some users are under this business.please delete the users to delete this business'
            );
    ...
    protected function DeleteAgency($AgencyID)
    {
        ...
        $objSuperUser = \Vokuro\Models\Users::findFirst(
            "agency_id = {$AgencyID} AND role='Super Admin'"
        );

        // Delete all business subscriptions under an agency
        $dbSubscriptionPricingPlan = \Vokuro\Models\SubscriptionPricingPlan::find(
            "user_id = {$objSuperUser->id}"
        );

        foreach ($dbSubscriptionPricingPlan as $objSubscriptionPricingPlan) {
            $dbSubscriptionPricingPlanParameterList = \Vokuro\Models\SubscriptionPricingPlanParameterList::find(
                "subscription_pricing_plan_id = {$objSubscriptionPricingPlan->id}"
            );
        ...
        $dbBusinesses = \Vokuro\Models\Agency::find(
            "parent_id = {$AgencyID}"
        );
        ...
        try {
            ...
            if ($objBusiness) {
                ...
                if (!$this->getPermissions()->canUserEditAgency($this->getUserObject(), $objBusiness)) {
                    throw new \Exception(
                        "You do not have permissions to edit/delete this agency with the id of: {$objBusiness->agency_id} with the user id of{$UserID}"
                    );
                }
            }
            ...
            if (!$objBusiness) {
                $this->flash->error(
                    "The " . ($agency_type_id == 1 ? 'agency' : 'business') . " was not found"
                );
    ...
    public function confirmationAction($agency_type_id, $agency_id, $user_id)
    {
        ...
        $this->response->redirect(
            '/admindashboard/view/' . $agency_type_id . '/' . $agency_id . '?s=2'
        );
    ...
    {
        if ($agency_type_id && !is_numeric($agency_type_id)) {
            throw new \Exception('invalid agency type id provided');
        }

        if ($agency_id && !is_numeric($agency_id)) {
            throw new \Exception('invalid agency id provided');
        }
        ...
        if ($objUser->is_admin || $objEntity->parent_id == \Vokuro\Models\Agency::BUSINESS_UNDER_RV
    ...
    public function editAction($agency_id)
    {
        if (!is_numeric($agency_id)) {
            throw new \Exception('Invalid agency id provided, expected integer');
        }
    ...
    public function forgotPasswordAction($agency_type_id, $agency_id, $user_id)
    {
        ...
        if ($resetPassword->save()) {
            $this->flash->success(
                'Success! Have the employee check their email for a reset password message'
            );
        } else {
