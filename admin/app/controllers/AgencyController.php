...
class AgencyController extends ControllerBusinessBase
{
    ...
    public function upgradePlanAction()
    {
        ...
        try {
            ...
            if ($identity) {
                ...
                $createdAgencySubscription = $SubscriptionManager->createAgencySubscription(
                    $objUser->id,
                    $tPricingInfo['PlanID'],
                    $tPricingInfo['RecurringPayment']
                );

                if ($createdAgencySubscription) {
                    $responseParameters['status'] = true;
                } else {
    ...
    public function findBusinessescustom()
    {
        ...
        foreach($tAgencies as $agency) {
            $result = $this->db->query(
                "SELECT * FROM `users` WHERE `agency_id`='". $agency->agency_id . "'"
            );
    ...
    public function indexAction()
    {
        ...
        $objAgencyPricingPlan = \Vokuro\Models\AgencySubscriptionPlan::findFirst(
    ...
    public function assignnumberAction($id)
    {
        ...
        if (($Twillioset['twilio_api_key'] == "" || $twilio_api_key==NULL)
            || ($Twillioset['twilio_auth_token'] == "" || $Twillioset['twilio_auth_token'] == NULL)) {
    ...
    public function agencygetTwilioDetails()
    {
        ...
        if ($agency) {
            ...
            if ($twilio_api_key == "" 
                ...
                $agency1 = Agency::findFirst(
                    array($conditions, "bind" => $parameters1)
                );

                $conditionsUser = "agency_id = :agency_id:";
                $userParam = $parameters = array("agency_id" => $agency1->agency_id);
                
                $userObjNew = Users::findFirst(
                    array($conditionsUser, "bind" => $userParam)
                );
    ...
    public function customnumberAction()
    {
        ...
        {
            $usersinfo = \Vokuro\Models\users::findfirst(
                "agency_id = " . $agent->id
            );
            
            $result = $this->db->query(
                "SELECT * FROM `twilio_number_to_business` WHERE `buisness_id`='" . $usersinfo->id . "'"
            );
    ...
    public function emailisexistAction()
    {
        ...
        $userObj = \Vokuro\Models\Users::findFirst(
            "email like '" . $email . "'"
        );

        $agencyObj = \Vokuro\Models\Agency::findFirst(
            "email like '" . $email . "'"
        );
