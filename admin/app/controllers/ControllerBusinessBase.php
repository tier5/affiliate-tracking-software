...
class ControllerBusinessBase extends ControllerBase
{
    ...
    public function editAction($agency_id = 0) 
    {
        ...
        if ($this->request->isPost()) {
            ...
            $params = [
                ...
                'email'              => $this->request
                                             ->getPost('email', 'striptags'),
                'address'            => $this->request
                                             ->getPost('address', 'striptags'),
                'locality'           => $this->request
                                             ->getPost('locality', 'striptags'),
                'state_province'     => $this->request
                                             ->getPost('state_province', 'striptags'),
                'postal_code'        => $this->request
                                             ->getPost('postal_code', 'striptags'),
                'country'            => ($this->request
                                              ->getPost('country', 'striptags')) ? $this->request->getPost('country', 'striptags'): 'n/a',
                'phone'              => $this->request
                                             ->getPost('phone', 'striptags'),
                'date_created'       => (isset($age->date_created) ? $age->date_created : date('Y-m-d H:i:s')),
                'subscription_id'    => $this->request
                                             ->getPost('subscription_pricing_plan_id', 'striptags'),
            ...
            $this->flash
                 ->success("The {$CreateType} was " . ($agency_id > 0 ? 'edited' : 'created') . " successfully");
            $this->flash
                 ->success('A confirmation email has been sent to ' . $this->request->getPost('admin_email'));
            if(!$errors) { 
                $db->commit();
            }
            if($errors) { 
    ...
    public function createAction($agency_type_id, $agency_id = 0, $parent_id = 0) 
    {
        ...
        // if post request create business/agency
        if ($this->request->isPost()) {
            //print_r($_POST);exit;
            $errors = [];
            $messages = [];
            ...
            if ($errors) {
                ...
                $this->flash
                     ->error("There was an error creating the {$CreateType}<BR />" . implode("<BR />", $errors));
            ...
            $this->flash
                 ->success("The " . ($age->agency_type_id == 1 ? 'agency' : 'business') . " was created successfully");
            $this->flash
                 ->success('A confirmation email has been sent to ' . $this->request->getPost('admin_email'));
            //if(!$errors) $db->commit();
            if ($errors) {
                $db->rollback();
                $this->flash
                     ->error("There was an error creating <BR />" . implode("<BR />", $errors));
                return false;
            }
    ...
    public function viewAction($agency_type_id, $agency_id = 0) 
    {
        ...
        if (isset($_GET['s']) && $_GET['s'] == 1) {
            $this->flash
                 ->success('Success! Have the employee check their email for a reset password message');
        }
        if (isset($_GET['s']) && $_GET['s'] == 2) {
            $this->flash
                 ->success('Success! Have the employee check their email for a confirmation message');
        }
        if (isset($_GET['s']) && $_GET['s'] == 3) {
            $this->flash
                 ->success('This user has not activated their account, please activate or resend the confirmation email');
        }
    }
    ...
    protected function createSubscriptionPlan($user, $request) 
    {
        ...
        return $this->di
                    ...
                    ->createSubscriptionPlan($newSubscriptionParameters);
    }

    /**
     * Update subscription locations and sms
     *
     * @param  object $user
