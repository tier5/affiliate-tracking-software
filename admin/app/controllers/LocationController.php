...
class LocationController extends ControllerBase
{
    ...
    protected function disconnectReviewSite($LocationID, $Type)
    {
        ...
        $objLocationReviewSite = \Vokuro\Models\LocationReviewSite::findFirst(
            "location_id = {$LocationID} AND review_site_id = {$Type}"
        );
    ...
    {
        $BusinessID = $this->transliterateString($BusinessID);
    ...
    {
        $objReviewsService = new \Vokuro\Services\Reviews();
        $objGoogleBusiness = $objReviewsService->getGoogleMyBusinessData(
            $LocationID,
            $BusinessID
        );
    ...
    public function pickFacebookBusinessAction($BusinessID, $LocationID, $RedirectToSession = 0)
    {
        ...
        if (!$Picked) {
            $this->flash->error(
                "Your business could not be found in the subsequent facebook search.  Please contact customer support."
            );
        }
    ...
    {
        $dbArray = Review::find(
            "rating_type_id = 1 AND location_id = {$LocationID}"
        );
    ...
    public function updateLocationAction()
    {
        ...
        $objLocationReviewSite = LocationReviewSite::findFirst(
            "location_id = {$location_id} AND review_site_id = " . \Vokuro\Models\Location::TYPE_YELP
        );
    ...
    protected function updateAgencySubscriptionPlan($LocationID, $Creating)
    {
        ...
        $SubscriptionLevel = $objSubscriptionManager->GetBusinessSubscriptionLevel(
            $objLocation->agency_id
        );
    ...
    public function deleteAction($location_id)
    {
        ...
        if (!$objLocation->delete()) {
            ...
            $objFirstLocation = \Vokuro\Models\Location::findFirst(
                "agency_id = {$agency_id_to_check}"
            );
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($objAgency->parent_id == \Vokuro\Models\Agency::BUSINESS_UNDER_RV) {
                ...
                if (!$objParentAgency->email_from_address && !$objParentAgency->custom_domain) {
                    throw new \Exception(
                        "Your email from address or your custom domain needs to be set to send email"
                    );
                }
