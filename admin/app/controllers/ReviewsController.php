...
class ReviewsController extends ControllerBase
{
        ...
        if ($agency) {
            ...
            if ($agency->parent_id == Agency::BUSINESS_UNDER_RV) {
                ...
                if (isset($agency->twilio_api_key) 
                    ...
                    if (isset($agency1->twilio_api_key) 
                        && $agency1->twilio_api_key != "" 
                        && isset($agency1->twilio_auth_token) 
                        && $agency1->twilio_auth_token != "" 
                        && isset($agency1->twilio_from_phone) 
                        && $agency1->twilio_from_phone != "") {
        ...
        if (!empty($_POST)) {
            ...
            if (isset($_POST['formposttype']) && $_POST['formposttype'] == 'test') {
                ...
                if (!empty($_POST['review_invite_ids'])) {
                    ...
                    {
                        $this->flash->success(
                            "You have exceeded the the limit of sending non viral messages."
                        );
                    }
    ...
    public function sent_message_viewAction($id = 0)
    {
        ...
        $sms_broadcast = SMSBroadcast::findFirst(
            array($conditions, "bind" => $parameters)
        );
        ...
        $this->view->invitelist = ReviewInvite::find(
            array($conditions, "bind" => $parameters)
        );
