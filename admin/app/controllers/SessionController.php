...
class SessionController extends ControllerBase
{
  ...
  {
    $this->view->setTemplateBefore('login');
    $this->tag->setTitle('Review Velocity | Test Auth');
    ...
    /*
    START Credit Card Processing Test
    // Subscription Type Info
    $subscription = new AnetAPI\ARBSubscriptionType();
    $subscription->setName("Review Velocity");
    ...
    //END Credit Card Processing Test
    */


    //START: subscription check test
    $request = new AnetAPI\ARBGetSubscriptionStatusRequest();
    $request->setMerchantAuthentication($merchantAuthentication);
    $request->setRefId($refId);
    $request->setSubscriptionId("3073976");
  ...
  //echo '<pre>$customer:'.print_r($customer,true).'</pre>';
              ...
              if (!$isemailunuique) $this->flash->error('That email address is already taken.');
              if (!$ccformvalid) $this->flash->error('Please enter your credit card information.');
            }
  ...
  public function signupAction($subscription_id = 0)
  {
    ...
    if ($this->request->isPost()) {
      $ccformvalid = true;
      if (isset($subscriptionobj->subscription_id) == true && $subscriptionobj->subscription_id > 0) $ccformvalid = $ccform->isValid($this->request->getPost());
      ...
      if ($uservalid && $ccformvalid && $isemailunuique) {
        ...
        if (isset($subscriptionobj->subscription_id) == true && $subscriptionobj->subscription_id > 0) {
          ...
          if (isset($subscription->trial_length) && $subscription->trial_length > 0) $paymentSchedule->setTrialOccurrences($subscription->trial_length);

          $subscription->setPaymentSchedule($paymentSchedule);
          $subscription->setAmount($subscriptionobj->amount);
          //use this for free trial
          if (isset($subscription->trial_length) && $subscription->trial_length > 0) $subscription->setTrialAmount($subscription->trial_amount);
  ...
  public function loginAction()
  {
    ...
    if (isset($_GET['n']) && $_GET['n'] == 1) $this->flash->success('Your account has been created and a confirmation email has been sent to your email address.');
    $this->view->form = $form;
  }
