...
class TwilioController extends ControllerBusinessBase
{
    ...
    public function getAvailableNumberAction()
    {
        ...
        if ($number_type_select == "") {
            ...
            $client_get = new Services_Twilio(
                $Twillioset['twilio_api_key'],
                $Twillioset['twilio_auth_token']
            ); 
    ...
    public function getPreviousNumberAction()
    {
        ...
        foreach ($client->account->incoming_phone_numbers as $numberx) {
            $result = $this->db->query(
                "SELECT * FROM `twilio_number_to_business` WHERE `phone_number`='" . $numberx->phone_number . "'"
            );
    ...
    public function bookpurchasedAction($numbers)
    {
        ...
        $result = $this->db->query(
            "INSERT INTO twilio_number_to_business ( `friendly_name`, `phone_number`, `buisness_id`, `created`,`updated`,`parent_twilio_api_key`,`parent_twilio_auth_token`,`parent_user_id`,`purchased`,`twilio_purchase_token`) VALUES ( '".$friendly_number."', '".$number."', '".$id."','".$createdxx."','".$createdxx."','".$twilio_api_key."','".$twilio_auth_token."','".$twilio_user_id."','0','')"
        );
    ...
    public function releseThisnumberAction($numbers)
    {
        ...
        $result = $this->db->query(
            "DELETE FROM `twilio_number_to_business` WHERE `phone_number`='" . $number . "'"
        );
    ...
    public function agencyreleseThisnumberAction($numbers)
    {
        ...
        $this->response->redirect('/agency/customnumber');
        $this->view->disable();
        return;
    }

    public function booknumberagencyAction($numbers,$user_id)
    {
        $pieces = explode("||", $numbers);
        $number = base64_decode($pieces[0]);
    ...
    public function getPreviousNumberAgencyAction()
    {
        $this->view->setTemplateBefore('ajax');
        $number['local'] = array();
        $number['Mobile'] = array();
    ...
    public function getAvailableNumberagencyAction()
    {
        $this->view->setTemplateBefore('ajax');
        $number['local'] = array();
        $number['Mobile'] = array();
        ...
        $this->view->pick("twilio/getlistagency");
    }
}