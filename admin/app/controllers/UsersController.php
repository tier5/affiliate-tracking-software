...
class UsersController extends ControllerBase
{
    ...
    public function linkAction($uid)
    {
        ...
        if (!empty($userObj)) {
            foreach ($userObj as $obj)
            {
                ...
                $userObj1 = Location::findFirst(
                    array($conditions1, "bind" => $parameters1)
                ); 
    ...
    public function editFunction($id, $profilesId)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!$user->save()) {
                ...
                $this->flash->success(
                    "The ".($user->profilesId==3?'user':'admin user')." was updated successfully"
                );
    ...
    public function deleteFunction($id, $profilesId)
    {
        ...
        if (!$user) {
            $this->flash->error(
                "The " . ($profilesId == 3 ? 'user' : 'admin user') . " was not found"
            );
            ...
            if (!$form->isValid($this->request->getPost())) {
                ...
                if (!$passwordChange->save()) {
                    ...
                    if ($_SESSION['toemail_log']) {
                        ...
                        $feed_back_body .= "<br>" . $_SESSION['AgencyUser_log'] . "<br>" . $_SESSION['Agencyname_log'];

                        $mail = new Email();
                        $mail->sendLoginDetailsEmployee(
                            $_SESSION['confirm_user_id'],
