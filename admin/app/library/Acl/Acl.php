...
class Acl extends Component
{
    ...
    {
      if (isset($_SERVER['REQUEST_URI']) && (strpos($_SERVER['REQUEST_URI'],'cron') !== false || strpos($_SERVER['REQUEST_URI'],'send_review_invite') !== false)) return false;
    ...
    public function isAllowed($profile, $controller, $action)
    {
      ...
      //employees can not manage users
      if (($profile=='Agency Admin' || $profile=='Business Admin') && $controller=='users') {
        return true;
      } else {
        return true;
      }
...
//echo '<pre>$this->acl:'.print_r($this->acl,true).'</pre>';
            return $this->acl;
        }
        ...
        if (function_exists('apc_fetch')) {
            ...
            if (is_object($acl)) {
                $this->acl = $acl;
//echo '<pre>$acl:'.print_r($acl,true).'</pre>';
                return $acl;
            }
        }

        // Check if the ACL is already generated
//echo '<pre>$this->filePath:'.print_r($this->filePath,true).'</pre>';
