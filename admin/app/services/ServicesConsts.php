<?php

namespace Vokuro\Services;

class ServicesConsts {

    static $PAYMENT_PROVIDER_AUTHORIZE_DOT_NET = "AuthorizeDotNet";

    static $PAYMENT_PROVIDER_STRIPE = "Stripe";

    static $PAYMENT_PLAN_FREE = 'FR';

    static $PAYMENT_PLAN_TRIAL = 'TR';

    static $PAYMENT_PLAN_PAID = "PD";

    static $PAYMENT_PLAN_MONTHLY = 'Monthly';

    static $PAYMENT_PLAN_YEARLY = 'Annually';

    static $TRIAL_PLAN_LOCATIONS = 1;

    static $TRIAL_PLAN_MESSAGES = 100;

    static $YELP_REVIEW_TYPE = 1;

    static $FACEBOOK_REVIEW_TYPE = 2;

    static $GOOGLE_REVIEW_TYPE = 3;
}



