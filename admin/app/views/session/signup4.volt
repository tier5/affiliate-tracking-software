...
<form class="register-form4" action="<?=((strpos($_SERVER['REQUEST_URI'],'location')>0)?'/location/create3/'.$location_id:'/session/signup4/'.(isset($subscription->subscription_id)?$subscription->subscription_id:'').(isset($_GET['code'])?'?code='.$_GET['code']:''))?>" method="post" style="display: block;">
    ...
    <div class="top-row">
        <div class="form-group col-md-6">
            <label for="review_goal" class="col-md-8 control-label">How many reviews do you want
                monthly<span class="required">*</span>:</label>
            <div class="col-md-4">
        ...
        <div class="form-group col-md-6">
            <label for="lifetime_value_customer" class="col-md-8 control-label">What is the lifetime value of a
                customer:</label>
            <div class="col-md-4">
    ...
    <div class="col-md-12 employeetable">
        <div class="middle-row" style="border-bottom: 1px solid #E7ECF0;">
            <a href="#" id="createlink" class="btnLink btnSecondary" style="height: 32px !important; line-height: 16px; padding-left: 15px; padding-right: 15px;">Create
                Employee</a>
            <?php
  if (strpos($_SERVER['REQUEST_URI'],'location')>0 && isset($employees) && count($employees) > 0) {
            ?>
            <a href="#" id="selectlink" class="btnLink btnSecondary" style="height: 32px !important; line-height: 16px; padding-left: 15px; padding-right: 15px; margin-right: 20px;">Select
                Employees</a>
            <?php
  }
  ?>
        ...
        {% for user in employees %}
            {% if loop.first %}
                ...
                <tr role="row">
                    ...
                    <th class="" tabindex="0" aria-controls="basic-datatables" rowspan="1" colspan="1" style="width: 195px;" aria-label="Email: activate to sort column ascending">
                        Email Address
                    </th>
                    <th class="" tabindex="0" aria-controls="basic-datatables" rowspan="1" colspan="1" style="width: 90px;" aria-label="Cell Phone: activate to sort column ascending">
                        Cell Phone
                    </th>
                </tr>
                </thead>
    ...
    <div class="form-actions">
        <button type="submit" id="register-submit-btn" class="btnsignup uppercase"><?=(strpos($_SERVER['REQUEST_URI'],'location')>
            0?'Save':'Next: Invite Friends')?>
        </button>
    </div>
...
<div id="page-wrapper" class="create" style="display: none;">
    <form id="createemployeeform" class="register-form4" action="<?=((strpos($_SERVER['REQUEST_URI'],'location')>0)?'/users/createemployee2/'.$location_id:'/users/createemployee/'.(isset($subscription->subscription_id)?$subscription->subscription_id:'').(isset($_GET['code'])?'?code='.$_GET['code']:''))?>" method="post" style="display: block; padding-top: 0;">
        ...
        <div class="col-md-12">
            ...
            <div class="row" style="clear: both;">
                <div class="field">
                    <button id="createuser" type="submit" class="btnLink btnSecondary">Save</button>
                </div>
            </div>
            <div style="clear: both;">&nbsp;</div>
        </div>
...
<div id="page-wrapper2" class="create" style="display: none;">
    <form id="selectemployeeform" class="register-form4" action="/location/selectemployees/<?=$location_id?>" method="post" style="display: block; padding-top: 0;">
        <div class="closelink close"></div>
        <div class="col-md-12">
            <div class="title"><h3 style="margin-bottom: 0; margin-top: 0; padding-bottom: 5px;">Select Employees</h3>
            </div>
    ...
    if (isset($employees) && count($employees) > 0) {
            ...
            foreach($user->locations as $location) {
                        ...
                        ?>
                        <span class="name" style="display: inline-block; min-width: 20px;"><input type="checkbox" name="employees[]" value="<?=$user->id?>" <?=($checked?'checked="checked"':'')?>
                            /></span>
                        <span style="min-width: 200px; padding: 0 0 0 5px; display: inline-block;"><?=$user->
                            name?></span>
                    </div>
                </div>
                <?php
    ...
    ?>
            ...
            <div class="row" style="clear: both;">
                <div class="field">
                    <button id="selectuser" type="submit" class="btnLink btnSecondary">Save</button>
                </div>
            </div>
            <div style="clear: both;">&nbsp;</div>
        </div>
