...
class LocationImporter extends \Vokuro\Controllers\LocationController {
    ...
    public function initialize()
    {
        ...
        file_put_contents(__DIR__ . '../admin/cron_review_log', $dateTime . "\r\n", FILE_APPEND);
        $path_to_admin = realpath(__DIR__ . '/../');
        require_once $path_to_admin . '/app/library/Google/mybusiness/Mybusiness.php';
    ...
    public function ImportGoogleMyBusinessReviews()
    {
        $dbLocations = \Vokuro\Models\LocationReviewSite::find(
            "review_site_id = " . \Vokuro\Models\Location::TYPE_GOOGLE . " AND json_access_token IS NOT NULL AND json_access_token != ''"
        );
    ...
    {
        $dbLocations = \Vokuro\Models\LocationReviewSite::find(
            "review_site_id = " . \Vokuro\Models\Location::TYPE_FACEBOOK . " AND access_token IS NOT NULL AND access_token != ''"
        );
    ...
    {
        $dbLocations = \Vokuro\Models\LocationReviewSite::find(
            "review_site_id = " . \Vokuro\Models\Location::TYPE_YELP . " AND external_location_id IS NOT NULL AND external_location_id != ''"
        );
