...
$affiliate_details=$db->prepare("select id from idevaff_payments where record = ?");
$affiliate_details->execute(array($invoice_id));
$affiliate_details=$affiliate_details->fetch();
$affiliate_id=$affiliate_details['id'];

$alldata=$db->prepare("select * from idevaff_affiliates where id = ?");
$alldata->execute(array($affiliate_id));
...
$get_tax->execute(array($affiliate_id));
...
$earnings2 = $db->prepare("select code, amount from idevaff_payments where record = ?"); 
$earnings2->execute(array($invoice_id));
$row2 = $earnings2->fetch();
$pexact = $row2['amount'];
$pdatedisplay = date('m-d-Y', $row2['code']);

$origtotal = $db->prepare("select SUM(payment) AS totalorig from idevaff_archive where payment_rec = ?"); 
$origtotal->execute(array($invoice_id));
$orig_total =  $origtotal->fetch();
$pexacttotalorig = $orig_total['totalorig'];
$pexacttotalorigs = number_format($pexacttotalorig,$decimal_symbols);

$debittotal = $db->prepare("select SUM(amount) AS totaldebs from idevaff_debit_archive where payment_record = ?"); 
$debittotal->execute(array($invoice_id));
...
$amount_to_pay_grand_total = $pexacttotalorig - $pexacttotaldebsd;
...
//$check_vat_valid = $db->query("SELECT COUNT(*) FROM idevaff_affiliates where id = '" . $affiliate_id . "' and vat_override = '1'");
$check_vat_valid = $db->prepare("SELECT COUNT(*) FROM idevaff_affiliates where id = ? and vat_override = '1'");
$check_vat_valid->execute(array($affiliate_id));
if (!$check_vat_valid->fetchColumn()) {
$vat_amount = ($pexact / $vat_value_extended) * $vat_value;
} }
?>
...
.commission h1{color:#333;font-size:28px;font-weight:600;line-height:32px;margin:0;text-transform:uppercase;}
	.image_paid  {
  background: url("images/paid.png") no-repeat 90% 0px;
  height: 85px;
  width: 100%;
  margin-top: -87px;
  margin-bottom: 5px;
...
<div class="main_container">
	...
	<div class="row">
		<table class="table_invoice" border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<tbody>
				<tr>
					<td width="25%" class="grey">Affiliate ID</td>
					<td width="75%"><?PHP echo $affiliate_id; ?></td>
				</tr>
				<tr>
					<td width="25%" class="grey">Username</td>
					<td width="75%"><?PHP echo $uname; ?></td>
				</tr>
				<tr>
					<td width="25%" class="grey">Phone Number</td>
					<td width="75%"><?PHP echo $phone; ?></td>
				</tr>
				<tr>
					<td width="25%" class="grey">Payment Preference</td>
					<td width="75%"><?PHP if ($pp == 1) { print "PayPal</font>"; } elseif ($pp == 0) { print "Paper Check"; } ?></td>
				</tr>
				<tr>
					<td width="25%" class="grey">Tax ID, SSN or VAT</td>
					<td width="75%"><?PHP if ($utax) { print $utax; } else { print "N/A"; } ?></td>
				</tr>
			</tbody>
		</table>
	</div>
...
$getvars->execute(array($group_id, $affiliate_id));
	...
	<div class="row">
	   <table class="table_invoice" border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<td width="30%">Payment Date</td>
				<td width="70%"><?PHP echo $pdatedisplay; ?></td>
			</tr>
			<tr>
				<td width="30%"><?PHP if ($pexacttotaldebs <= 0) { ?><font color="#CC0000"><?PHP } ?>Payment Amount<?PHP if ($pexacttotaldebs <= 0) { ?></font><?PHP } ?></td>
				<td width="70%"><?PHP if ($pexacttotaldebs <= 0) { ?><font color="#CC0000"><?PHP } ?><?PHP if($cur_sym_location == 1) { echo $cur_sym; } echo number_format($pexacttotalorig,$decimal_symbols); if($cur_sym_location == 2) { echo " " . $cur_sym . " "; }echo " $currency"; ?><?PHP if ($pexacttotaldebs <= 0) { ?><?PHP } ?></td>
			</tr>
<?PHP if ($pexacttotaldebs > 0) { ?>
			<tr>
				<td width="30%">Minus Debit Amount</td>
				<td width="70%"><?PHP if($cur_sym_location == 1) { echo $cur_sym; } echo $pexacttotaldebsd; if($cur_sym_location == 2) { echo " " . $cur_sym . " "; }echo " $currency"; ?></td>
			</tr>
			<tr>
				<td width="30%"><font color="#CC0000">Revised Payment Amount</font></td>
				<td width="70%"><font color="#CC0000">
					<?PHP if($cur_sym_location == 1) { echo $cur_sym; } echo $pexact; if($cur_sym_location == 2) { echo " " . $cur_sym . " "; }echo " $currency"; ?>
					</font></td>
			</tr>
<?PHP } ?>
...
<?PHP if ($pexacttotaldebs > 0) { ?>
	<div class="row">
		...
		<table class="table_invoice" border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<tbody>
<?PHP
$acct = $db->prepare("select * from idevaff_debit_archive where payment_record = ? ORDER BY id"); 
$acct->execute(array($invoice_id));
...
<?PHP } ?>
	...
	<div class="row">
		<table class="table_invoice" border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<tbody>
				<?PHP
$acct = $db->prepare("select * from idevaff_archive where payment_rec = ? ORDER BY record"); 
$acct->execute(array($invoice_id));
//while ($qry = $acct->fetch()) {
foreach ($acct->fetchAll() as $qry) {
...
$earnings2_total = $db->prepare("select SUM(payment) AS total from idevaff_archive where payment_rec = ?"); 
$earnings2_total->execute(array($invoice_id));
$row2_total = $earnings2_total->fetch();
$pexact = $row2_total['total'];
	...
	<div class="row">
		...
		<div class="fright">
			<div class="commission">
				...
				<h1>
					<?PHP if($cur_sym_location == 1) { echo $cur_sym; } echo $amount_to_pay_grand_total; if($cur_sym_location == 2) { echo " " . $cur_sym . " "; }echo " $currency"; ?>
				</h1>
			</div>
		</div>
	</div>
