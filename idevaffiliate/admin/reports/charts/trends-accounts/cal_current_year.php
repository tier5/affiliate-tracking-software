<?php
$total_month = 12;
$total_month = $total_month - ($total_month - date('m'));
$record_array= array();

for($m=0; $m<$total_month;$m++)
{
	$startTime	= mktime(0, 0, 0, $total_month-$m , 1, date('Y'));    
	$endTime 	= mktime(23, 59, 59, $total_month-$m, cal_days_in_month ( CAL_GREGORIAN, $total_month-$m, date('Y') ), date('Y'));
	
	//echo date('Y-m-d H:i:s', $startTime)  . " - ". date('Y-m-d H:i:s', $endTime);echo "<br>";
